...
class RLGCDeconvolver:
    ...
    def deconvolve(
        ...
        # RLGC uses median(image) as init_value for full-frame path.
        # For sparse images (mostly zeros), median=0 causes algorithm to fail.
        # Add small background offset to ensure non-zero initialization.
        background_offset = 0.0
        if np.median(image) == 0 and image.max() > 0:
            # Add 1% of max as background offset
            background_offset = image.max() * 0.01
            image = image + background_offset
        ...
        # Remove the background offset from result
        if background_offset > 0:
            result = np.clip(result - background_offset, 0, None)
